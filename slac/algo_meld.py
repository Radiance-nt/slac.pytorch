...
class MeldAlgorithm:
    ...
    def create_replaybuffer(self, num):
        # Replay buffer.
        self.buffer = [ReplayBuffer(self.buffer_size, self.num_sequences,
                                    self.state_shape, self.action_shape, self.device) for i in range(20)]
    ...
    def step(self, env, ob, t, is_random, task_idx):
        t += 1
        ...
        self.buffer[task_idx].append(action, reward, mask, state, done)

        if done:
            t = 0
            state = env.reset()
            ob.reset_episode(state)
            self.buffer[task_idx].reset_episode(state)

        return t

    def update_latent(self, writer, task_idx):
        self.learning_steps_latent += 1
        state_, action_, reward_, done_ = self.buffer[task_idx].sample_latent(self.batch_size_latent)
    ...
    def update_sac(self, writer):
        ...
        buffer = random.choice(self.buffer)
        state_, action_, reward, done = buffer.sample_sac(self.batch_size_sac)
